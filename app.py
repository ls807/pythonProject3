...
if 'page' not in st.session_state:
    ...
    st.session_state.regret_scores = []  # 新增后悔程度分数
...
def instructions_page():
    ...
    **任务目标：**
    ...
    您需要<span style="color: blue">扮演银行贷款审批人员</span>，根据申请者的资料判断贷款<span style="color: blue">是否会被批准</span>。

    **实验流程：**

    1. 您将接受一个简单的培训，了解实验平台的使用以及 AI 解释。
    2. 培训结束后，需回答几个问题，全部答对后才能进入正式实验。
    3. 正式实验中，您将依次评估20个贷款申请案例（前10个无解释，后10个<span style="color: blue">视组别而定</span>）。
    4. 实验中模型预测的准确率控制在70%。
    ...
    **报酬方式：**
    ...
    if group == 'group1':
        ...
        **文本解释示例：**
        ...
        st.write("您将根据申请者信息和AI的建议以及交互式解释，判断贷款是否会被批准")
...
def quiz_page():
    ...
    if group == 'group1':
        ...
        questions = [
            {
                'question': '在实验中，您需要根据申请者的资料和AI的建议以及交互式解释判断贷款是否会被批准。',
                'options': ['正确', '错误'],
                'answer': '正确'
            },
            {
    ...
    if no_explain:
        ...
        if group == 'group1':
            ...
            for col in X_case.columns:
                ...
                adjusted_value = st.slider(col, float(min_val), float(max_val), float(mean_val),
                                           key=f'adjust_{col}_{st.session_state.case_index}')
                if col == '贷款期限':
    ...
    if st.session_state.current_step == 1:
        with st.form(key='decision_form'):
            final_decision = st.radio("请给出您的最终决策：", ['批准', '拒绝'],
                                      key=f'final_decision_{st.session_state.case_index}')
        ...
        with st.form(key='scores_form'):
            ...
            # 新增后悔程度题目
            st.write("您对自己所做决策的后悔程度为？")
            regret_score = st.slider("", 0, 100, key=f'regret_score_{st.session_state.case_index}')
        ...
        if submit_scores:
            ...
            st.session_state.regret_scores.append(regret_score)  # 记录后悔分数
...
def thankyou_page():
    ...
    results = pd.DataFrame({
        ...
        'Regret_Score': st.session_state.regret_scores  # 新增后悔分数
    })
    ...
    # 将这四个统计标签加入 results DataFrame：新增一行统计汇总
